...
const MenuItemCard = () => {
  ...
  const [loadingItems, setLoadingItems] = useState([]);
   const { menuFilter } = useMenu();
  // Filter menu items based on the selected menu item
  const filteredMenuItems = menuFilter
    ? MenuItems.filter((item) => item.itemType.toLowerCase() === menuFilter.toLowerCase())
    : MenuItems;
    ...
    <>
      {filteredMenuItems.length >= 1 ? (
              ...
              <div key={id} className={`${backgroundColorClass} ${textColorClass} p-3 rounded-md flex flex-col gap-3 border-[1px] border-transparent hover:border-red-600 ease-in-out duration-500`}>
                ...
                <div className='flex justify-between items-center'>
                  ...
                  <>
                    ...
                    <p className='font-semibold' onClick={() => handleAddToCart(item)}>
                      Add to Cart
                    </p>
                  </div>
                </div>
        ...
        <div className='flex flex-col justify-center items-center gap-4 text-xl font-mono my-5'>
          <img src='/pizza-delivery.png' alt='' className='w-full' />
          <p className=' text-red-600'>No Food Found!</p>
        </div>
      )}
    </>
  );
